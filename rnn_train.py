"""
training mdn-rnn from pre-processed data
also save 1000 intial mu and logvar, for generative experiments (not related to training)
"""
import numpy as np
import os
import json
...
#tf.config.experimental_run__functions_eagerly(True) # used for debugging
...
#save 1000 initial mu and logvars. usee for sampling when training in dreams
...
def ds_gen():
    for _ in range(args.rnn_num_steps):
        ...
        #suboptimal b/c this experment only taking the first set of steps
...
for raw_z, raw_a, raw_r, raw_d, raw_N in dataset:
    ...
    z_targ = tf.concat([raw_z[:, 1:, :], dummy_zero], axis=1) #zero pad at the end but, it is not going to be used
    ...
    [tf.summary.scalar(loss_key, loss_val, step=step) for loss_key, loss_val in loss.d_targ
    ...
    if (step % 20 == 0 and step > 0):
        ...
        for loss_key, loss_val in loss.items():
            output_log += ', {}: {:.4f}'.format(loss_key, loss
        print(output_log)
