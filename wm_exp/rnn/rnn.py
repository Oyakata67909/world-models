...
# controls whther concatenate (z, c, h), etc for features used for car
MODE_ZCH = 0
MODE_ZC = 1
MODE_Z = 2
MODE_Z_HIDDEN = 3  # extra hidden layer
MODE_ZH = 4
...
def sample_vae(vae_mu, vae_logvar):
    ...
    z = tfp.layers.DistributionLambda(lambda theta: tfp.distributions.MultivariateNormalDiag(
        loc=theta[:, :sz], scale_diag=tf.exp(theta[:, sz:])), dtype=tf.float16)
    return z(mu_logvar)
...
class MDNRNN(tf.keras.Model):
    def __init__(self, args):
        ...
        self.optimizer = tf.keras.optimizer.Adam(
            learning_rate=self.args.rnn_learning_rate, clipvalue=self.args.rnn_grad_clip)
        ...
        rnn_out_size = args.rnn_num_mixture * args.z_size * 3 + \
            args.rnn_r_pred + args.rnn_d_pred  # 3 come from pi, mum std
        self.out_size = tf.keras.Sequential([
            tf.keras.layers.InputLayer(input_shape=self.args.rnn_size), 
            tf.keras.layers.Dense(rnn_out_size, name="mu_logsrd_logmix_net")])
    ...
    def get_loss(self):
        ...
        batch_size = self.atfs.rnn_batch_size
        ...
        def z_loss_func(y_true, y_pred):
            """
            this loss function is defined for N*; components each containing gaussian of 1 feature
            """
            ...
            out_mu, out_logstd, out_logpi = tf.split(mdnrnn_params, num_or_size_split=3, axis=1, name='mdn_coef_split')
            ...
            mask = tf.reshape(tf.tile(mask, [1, z_size]), [-1, 1]) #tile b/c we consider z_loss is flatten
            z_loss = mask * z_loss # dont train if episode ends
        ...
        def r_loss_func(y_true, y_pred):
            ...
            r_loss = tf.expand_dims(tf.keras.losses.MSE(
                y_true=r_true, y_pred=r_pred), axis=-1)
            r_loss = mask * r_loss
    ...
    def call(self, inputs, training=True):
        return self.__call___(inputs, training)
    def __call__(self, inputs, training=True):
        rnn_out, _, _ = self.inference_base(inputs, trianing=training)
        rnn_out = tf.reshape(rnn_out, [-1, self.args.rnn_size])
        out = self.out_net(rnn_out)
        mdnrnn_params, r, d_logits = self.parse_rnn_out(out)

        # can't output None b/c tfkeras redirrects to loss for optimization
        outputs = {'MDN': mdnrnn_params}
        if self.args.rnn_r_pred == 1:
            outputs['r'] = r
        if self.args.rnn_d_pred == 1:
            outputs['d'] = d_logits
...
def rnn_output(state, z, mode):
    ...
    if mode == MODE_ZCH:
        return np.concatenate([z, np.concaenate((state_c, state_h), axis=1)[0]])
    if mode == MODE_ZC:
...
def rnn_sim(rnn, z, states, a, training=True):
    ...
    d_dist = tfd.Bonomial(total_count=1, logits=d_logits)
