...
model_path_name = "results/{}/{}/tf.vae".format(args.exp_name, args.env_name)

if not os.path.exists(SERIES_DIR):
    os.makedirs(SERIES_DIR)

def ds_gen():
    filenames = os.listdir(DATA_DIR)[:10000] #最初の1万エピソードのみ
    n = len(filenames)
    for j, fname in enumerate(filenames):
        if not fname.endwith('npz'):
            continue
        ...
        n_pad = args.max_frames - img.shape[0] #padding so they are all 1000 step long episodes
...
def create_tf_dataset():
    dataset = tf.data.Dataset.from_generator(ds_gen, output_types=(tf.float32, tf.float32, tf.float32, tf.bool, tf.uint16), output_shape=((args.max_frames, 64, 64, 3), (args.max_frames, args.a_width), (args.max_frames,), (args.max_frames,), (args.max_frames,)))
    return dataset
...
def decode_batch(batch_z):
    #latent vectorをdecode
    batch_img = vae.decode(z.reshape(batch_size, z_size)) * 255.0
    batch_img = np.round(batch_img).astype(np.uint8)
    batch_img = batch_img.reshape(batch.size, 64, 64, 3)
    return batch_img
