...
from rl.wm_exp.rnn.rnn import MDNRNN
import numpy as np
import gym
import json
import os
...
class DoomTakeCoverMDRNN(DoomTakeCoverEnv):
    def __init__(self, args, render_mode=False, load_model=True, with_obs=False):
        super(DoomTakeCoverMDRNN, self).__init__()
        ...
        self.obs_size = self.rnn.args.z_size + \
        self.rnn.args.rnn_size * self.rnn.args.state_space
    ...
    def close(self):
        super(DoomTakeCoverMDRNN, self).close()
        tf.keras.backend.clear_session()
        gc.collect()
    def _step(self, action):
        ...
        obs, reward, done, _ = super(DoomTakeCoverMDRNN, self)._step(full_action)
    ...
    def _reset(self):
        obs = super(DoomTakeCoverMDRNN, self)._reset()
        small_obs = self._process_frame(obs)
        self.current_obs = small_obs
        self.rnn_states = rnn_init_state(self.rnn)
        self.restart = 1
        ...
        if self.with_obs:
            return [self._current_state(), self._current_obs]
        else:
    ...
    def _current_state(self):
        if self.rnn.args.state_space == 2:
            return np.concatenate([self.z, tf.keras.backend.flatten(self.rnn_state[1]), tf.keras.backend.flatten(self.rnn_stetes[0])], axis=0) #cell then hidden from some reason らしい
        return np.concatenate([self.z, tf.keras.backend.flatten(self.rnn_states[0])], axis=0) #hidden stateのみ
...
class DreamDoomTakeCoverMDNRNN:
    def __init__(self, args, render_mode=False, load_model=True):
        ...
        self.action_space = Box(low=-1.0, high=1.0, shape=(), dtype=np.float32)
        obs_size = self.rnn.args.z_size + self.rnn.args.rnn_size * self.rnn.args.state_space

        self_observation_space = Box(low=-50., high=50., shape=(obs_size,), dtype=np.float32)
        self.rnn_states = None
        self.o = None
        self._training = True
        self.seed()
    ...
    def reset(self):
        ...
        self.o = z
        z_ch = tf.concat([z, self._rnn_states[1], self.rnn_states[0]], axis=-1)
...
def make_env(args, dream_env=False, seed=-1 ,render_mode=False, full_episode=False, with_obs=False, load_model=True):
    if args.env_name == 'DoomTakeCover-v0':
        if dream_env:
            ...
            print('making rnn doom environment')
            env = DoomTakeCoverMDRNN(args=args, render_mode=render_mode, load_model=load_model, with_obs=with_obs)
        ...
        if dream_env:
            ...
            env = CarRacingMDRNN(args=args, full_episode=full_episode, with_obs=with_obs, load_model=load_model)
    if (seed >= 0):
        env.seed(seed)
    return env
