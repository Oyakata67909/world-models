...
#controls whther we concatenate (z, c, h) etc for features used for car.
MODE_ZCH = 0
MODE_ZC = 1
MODE_Z = 2
...
class Controller:
    """ simple one layers model for car racing """
    ...
    def get_model_params(self, stdev=0.1):
...
def simulate(controller, env, train_mode=False, render_mode=True, num_episode=5, seed=-1, max_len=-1):
    ...
    for episode in range(num_episode):
        ...
        if render_mode:
            print("total_reward", total_reward, "timesteps", t)
            env.close()
