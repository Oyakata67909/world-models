"""
random policyから生成された~200 episodesを保存
"""
...
for trial in range(args.max_trials):
    try:
        random_generated_int = random.randint(0, 2**21-1)
        ...
        if args.env_name == 'CarRacing-v0':
            controller.init_random_model_paramd(stdev=np.random.rand()*0.01)
        else:
