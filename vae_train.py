...
import random
import numpy as np
np.set_printoptions(precision=4, edgeitem=6, linewidth=100, suppress=True)
from vae.vae import CVAE
from utils import PARSER
args = PARSER.parse_args()
gpu_device = tf.config.experimental.list_physical_device('GPU')
for gpu in gpu_device:
    tf.config.experimental.set_memoty_growth(gpu, True)

def ds_gen():
    dirname = 'results/{}/{}/record'.fotmat(args.exp_name, args.env_name)
    filenames = os.lisdir(dirname)[:10000] #only use first 10k episodes
    n = len(filenames)
...
if __name__ == "__main__":
    ...
    tensorboard_dir = os.path.join(model_save_path, 'tenssorboard')
    summary_writer = tf.summary.create_file_writer(tensorboard_dir)
    summary_writer.set_as_default()
    tensorboard_callback = tf.keras.callbacks.Tensorboard(log_dir=tensorboard_dir, write_graph=False)
    shuffle_size = 20 * 1000 #only loads ~20 episodes for shuffle windows b/c in the referenced source code
    ...
    loss_weights = [1.0, 1.0] #weight both thr  reconstruction and KL loss the same
